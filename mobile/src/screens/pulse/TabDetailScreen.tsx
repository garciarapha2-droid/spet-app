/**
 * Tab Detail — view items, add items, close tab.
 */
...
export default function TabDetailScreen() {
  ...
  const loadItems = useCallback(async () => {
    ...
    try {
      const data = await tapService.getTabItems(session.id);
      setItems(data.items || []);
    } catch {
      setItems([]);
    }
  ...
  const handleClose = () => {
    Alert.alert(
      'Close Tab',
      `Close this tab? Total: $${Number(session.total).toFixed(2)}`,
      [
        { text: 'Cancel', style: 'cancel' },
        {
          text: 'Card',
          onPress: () => doClose('card'),
        },
        {
          text: 'Cash',
          onPress: () => doClose('cash'),
        },
        {
          text: 'Pix',
          onPress: () => doClose('pix'),
  ...
  const doClose = async (method: string) => {
    ...
    try {
      ...
      Alert.alert('Tab Closed', `Payment: ${method.toUpperCase()}`, [
  ...
  const tabNum = meta.tab_number || session?.tab_number;
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <FlatList
        ...
        ListHeaderComponent={
          <>
            ...
            <Card style={{ marginBottom: spacing.xxl, alignItems: 'center' }}>
              <Text style={{ fontSize: fontSize.xs, color: colors.textSecondary, textTransform: 'uppercase', letterSpacing: 1 }}>
                Running Total
              </Text>
              <Text style={{ fontSize: 36, fontWeight: '700', color: colors.primary, marginTop: spacing.sm, fontVariant: ['tabular-nums'] }}>
        ...
        renderItem={({ item }) => (
          ...
          >
            ...
            <Text style={{ fontSize: fontSize.md, fontWeight: '600', color: colors.text, fontVariant: ['tabular-nums'] }}>
        ...
        ListEmptyComponent={
          loading ? null : (
            <View style={{ alignItems: 'center', paddingTop: 20 }}>
              <Text style={{ fontSize: fontSize.md, color: colors.textSecondary }}>No items yet</Text>
            </View>
          )
        }
      />
      ...
      >
        <View style={{ flexDirection: 'row', gap: spacing.md }}>
          <Button
            title="Add Item"
            onPress={() => navigation.navigate('AddItem', { sessionId: session.id })}
            style={{ flex: 1 }}
          />
          <Button
            title="Close Tab"
