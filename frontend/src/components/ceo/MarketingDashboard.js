    ...
    <div data-testid="ceo-marketing">
            ...
            {funnel.map((f, i) => {
              ...
              return (
                <div key={f.stage} className="flex items-center gap-3" data-testid={`funnel-${f.stage.toLowerCase()}`}>
                  <span className="text-[10px] font-medium w-20 text-right" style={{ color: 'hsl(var(--text-secondary))' }}>{f.stage}</span>
                  ...
                  <span className="text-[9px] w-12" style={{ color: 'hsl(var(--text-tertiary))' }}>
                    ...
                    <div key={s.source}>
                      <div className="flex items-center justify-between mb-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] font-semibold text-foreground/80 capitalize">{s.source}</span>
                        </div>
                        <span className="text-[11px] font-bold text-foreground">{s.count} ({pct}%)</span>
                      </div>
      ...
      <ChartCard title="Monthly Lead Capture & Conversion" subtitle="Leads captured vs converted over time">
        ...
        <div className="flex items-center justify-center gap-6 mt-2 pt-2 border-t border-border/50">
          <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground font-medium"><span className="w-2.5 h-2.5 rounded-sm bg-slate-400" />Captured</span>
          <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground font-medium"><span className="w-2.5 h-2.5 rounded-sm bg-emerald-500" />Converted</span>
        </div>
      </ChartCard>
    </div>
  );
