...
# Base URL: https://manager-module.preview.emergentagent.com
...
```
https://manager-module.preview.emergentagent.com/auth/handoff?token=<JWT>
```

**Lovable code example:**
```javascript
// After successful login
const res = await fetch("https://manager-module.preview.emergentagent.com/api/auth/login", {
  method: "POST",
...
window.location.href = `https://manager-module.preview.emergentagent.com/auth/handoff?token=${data.access_token}`;
```
...
```javascript
// Step 1: Login
const loginRes = await fetch("https://manager-module.preview.emergentagent.com/api/auth/login", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ email, password }),
});
...
const handoffRes = await fetch("https://manager-module.preview.emergentagent.com/api/auth/handoff/create", {
  method: "POST",
...
window.location.href = `https://manager-module.preview.emergentagent.com/auth/handoff?code=${code}`;
```
...
| Purpose           | URL                                                                    |
|-------------------|------------------------------------------------------------------------|
| API base          | `https://manager-module.preview.emergentagent.com/api`               |
| App (frontend)    | `https://manager-module.preview.emergentagent.com`                   |
| Token handoff     | `https://manager-module.preview.emergentagent.com/auth/handoff?token=<JWT>` |
| Code handoff      | `https://manager-module.preview.emergentagent.com/auth/handoff?code=<CODE>` |
| Dashboard         | `https://manager-module.preview.emergentagent.com/venue/home`        |
