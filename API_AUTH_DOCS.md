...
# Base URL: https://spet-pos-refinement.preview.emergentagent.com
...
```
https://spet-pos-refinement.preview.emergentagent.com/auth/handoff?token=<JWT>
```

**Lovable code example:**
```javascript
// After successful login
const res = await fetch("https://spet-pos-refinement.preview.emergentagent.com/api/auth/login", {
  method: "POST",
...
window.location.href = `https://spet-pos-refinement.preview.emergentagent.com/auth/handoff?token=${data.access_token}`;
```
...
```javascript
// Step 1: Login
const loginRes = await fetch("https://spet-pos-refinement.preview.emergentagent.com/api/auth/login", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ email, password }),
});
...
const handoffRes = await fetch("https://spet-pos-refinement.preview.emergentagent.com/api/auth/handoff/create", {
  method: "POST",
...
window.location.href = `https://spet-pos-refinement.preview.emergentagent.com/auth/handoff?code=${code}`;
```
...
| Purpose           | URL                                                                    |
|-------------------|------------------------------------------------------------------------|
| API base          | `https://spet-pos-refinement.preview.emergentagent.com/api`               |
| App (frontend)    | `https://spet-pos-refinement.preview.emergentagent.com`                   |
| Token handoff     | `https://spet-pos-refinement.preview.emergentagent.com/auth/handoff?token=<JWT>` |
| Code handoff      | `https://spet-pos-refinement.preview.emergentagent.com/auth/handoff?code=<CODE>` |
| Dashboard         | `https://spet-pos-refinement.preview.emergentagent.com/venue/home`        |
