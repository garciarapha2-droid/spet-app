"""
Iteration 90: Mobile Dashboard API Tests
...
- Manager Dashboard: /api/manager/overview, /api/manager/staff, /api/manager/reports/sales
- Owner Dashboard: /api/owner/dashboard, /api/owner/finance
- Operational Modules: /api/tap/stats, /api/table/tables, /api/kds/tickets, /api/rewards/config
...
class TestCEODashboardAPIs:
    ...
    def test_ceo_health_returns_kpis(self, auth_headers):
        """GET /api/ceo/health returns kpis object with mrr, active_companies, active_venues, growth_pct"""
        ...
        expected_fields = ["mrr", "active_companies", "active_venues", "growth_pct"]
        ...
        assert isinstance(kpis["growth_pct"], (int, float)), f"growth_pct should be numeric"
        
        print(f"✓ CEO health KPIs: mrr={kpis['mrr']}, companies={kpis['active_companies']}, venues={kpis['active_venues']}, growth={kpis['growth_pct']}%")
    
    def test_ceo_revenue_returns_chart(self, auth_headers):
        """GET /api/ceo/revenue?period=month returns chart array with revenue, profit, fees"""
    ...
    def test_ceo_users_returns_array(self, auth_headers):
        """GET /api/ceo/users returns users array with id, email, status, roles"""
        ...
        user = users[0]
        expected_fields = ["id", "email", "status", "roles"]
        for field in expected_fields:
        ...
        print(f"✓ CEO users: {len(users)} users returned with id/email/status/roles")
...
class TestManagerDashboardAPIs:
    ...
    def test_manager_staff_returns_array(self, auth_headers):
        """GET /api/manager/staff?venue_id=... returns staff array"""
        response = requests.get(
    ...
    def test_manager_reports_sales_returns_data(self, auth_headers):
        """GET /api/manager/reports/sales?venue_id=... returns period, by_item, by_method"""
        response = requests.get(
...
class TestOwnerDashboardAPIs:
    ...
    def test_owner_dashboard_business_view(self, auth_headers):
        """GET /api/owner/dashboard?view=business returns kpis with revenue_mtd, venues array"""
        response = requests.get(
...
class TestOperationalModuleAPIs:
    ...
    def test_tap_stats_returns_data(self, auth_headers):
        """GET /api/tap/stats?venue_id=... returns tap stats"""
        response = requests.get(
    ...
    def test_kds_tickets_returns_array(self, auth_headers):
        """GET /api/kds/tickets?venue_id=... returns kitchen tickets"""
        response = requests.get(
...
class TestMobileServiceEndpoints:
    ...
    def test_tap_catalog(self, auth_headers):
        """GET /api/tap/catalog?venue_id=... returns catalog items"""
        response = requests.get(
    ...
    def test_manager_shifts(self, auth_headers):
        """GET /api/manager/shifts?venue_id=... returns shifts"""
        response = requests.get(
        ...
        assert isinstance(shifts, list), f"shifts should be array"
        
        print(f"✓ Manager shifts: {len(shifts)} shifts")
