...
class TestShiftOpsFeatures:
    """All Shift vs Operations tests in one class to share auth"""
    
    @pytest.fixture(autouse=True, scope="class")
    def setup_auth(self, request):
        """Get authentication token and set up session"""
        ...
        # Store session on class
        request.cls.session = requests.Session()
        request.cls.session.headers.update({
            "Authorization": f"Bearer {token}"
        })
        request.cls.token = token

    # ====== SHIFT OVERVIEW ======
    def test_01_shift_overview_returns_all_kpis(self):
        """Verify shift-overview returns revenue, tables_closed, avg_ticket, staff_cost, result, status"""
        response = self.session.get(f"{BASE_URL}/api/manager/shift-overview", params={
            "venue_id": VENUE_ID
        })
    ...
    def test_02_shift_overview_with_date_range(self):
    ...
    # ====== STAFF COSTS ======
    def test_03_staff_costs_returns_breakdown(self):
        """Verify staff-costs returns total_cost and staff breakdown"""
        response = self.session.get(f"{BASE_URL}/api/manager/staff-costs", params={
            "venue_id": VENUE_ID
        })
    ...
    # ====== STAFF CUSTOMIZE ======
    def test_04_customize_staff_hourly_rate(self):
        ...
        if staff:
            ...
            verify_response = self.session.get(f"{BASE_URL}/api/manager/staff-costs", params={
                "venue_id": VENUE_ID
            })
    ...
    # ====== STAFF ROLES ======
    def test_05_get_staff_roles(self):
        """Verify GET /api/manager/staff-roles returns roles"""
        response = self.session.get(f"{BASE_URL}/api/manager/staff-roles", params={
            "venue_id": VENUE_ID
        })
    ...
    def test_06_create_and_delete_staff_role(self):
        ...
        # Verify role exists
        list_response = self.session.get(f"{BASE_URL}/api/manager/staff-roles", params={
            "venue_id": VENUE_ID
        })
        ...
        verify_response = self.session.get(f"{BASE_URL}/api/manager/staff-roles", params={
            "venue_id": VENUE_ID
        })
    ...
    # ====== SHIFT HISTORY ======
    def test_07_shift_history_returns_daily_data(self):
        """Verify shift-history returns per-day performance"""
        response = self.session.get(f"{BASE_URL}/api/manager/shift-history", params={
            "venue_id": VENUE_ID,
            "days": 30
        })
        ...
        if data["history"]:
            ...
            print("\nShift History: No historical data available")

    # ====== SHIFT CHART ======
    def test_08_shift_chart_returns_daily_rev_cost(self):
        """Verify shift-chart returns revenue and cost per day"""
        response = self.session.get(f"{BASE_URL}/api/manager/shift-chart", params={
            "venue_id": VENUE_ID,
            "period": "30d"
        })
        ...
        if data["data"]:
            ...
            print("\nShift Chart: No data points available")

    def test_09_shift_chart_period_filters(self):
    ...
    # ====== SHIFT SNAPSHOT ======
    def test_10_shift_snapshot_saves_costs(self):
    ...
    # ====== SHIFT AI ======
    def test_11_shift_ai_generates_analysis(self):
        ...
        if insight.get("summary") != "LLM not configured":
            ...
            print("\nAI Analysis: LLM not configured (expected in test env)")
        
        print(f"Disclaimer present: {bool(data.get('disclaimer'))}")

    def test_12_shift_ai_with_question(self):
    ...
    # ====== REGRESSION TESTS ======
    def test_13_manager_overview_regression(self):
        """Verify Manager Overview endpoint works"""
        response = self.session.get(f"{BASE_URL}/api/manager/overview", params={
            "venue_id": VENUE_ID
        })
    ...
    def test_14_manager_staff_regression(self):
        """Verify Manager Staff endpoint works"""
        response = self.session.get(f"{BASE_URL}/api/manager/staff", params={
            "venue_id": VENUE_ID
        })
    ...
    def test_15_tables_by_server_regression(self):
        """Verify Tables by Server endpoint works"""
        response = self.session.get(f"{BASE_URL}/api/manager/tables-by-server", params={
            "venue_id": VENUE_ID
        })
